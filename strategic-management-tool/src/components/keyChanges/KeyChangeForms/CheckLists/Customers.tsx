...
const Customers: FC = () => {
  ...
  keyImpacts?.data[0]?.customers.data.forEach(item => {
  ...
  useEffect(() => {
    if (keyImpacts && keyImpacts.data && keyImpacts.data[0] && keyImpacts.data[0].customers) {
      initializeStates(keyImpacts.data[0].customers.data);
    }
  }, [keyImpacts]);

  const initializeStates = (customerData: CustomerData[]) => {
    ...
    setDivisions(customerData[0]?.divisions || []);
  }
