...
const Governance: FC = () => {
  ...
  keyImpacts?.data[0]?.governance_reporting.data.forEach(item => {
  ...
  useEffect(() => {
    if (keyImpacts && keyImpacts.data && keyImpacts.data[0] && keyImpacts.data[0].governance_reporting) {
      initializeStates(keyImpacts.data[0].governance_reporting.data);
    }
  }, [keyImpacts]);

  const initializeStates = (governanceData: GovernanceData[]) => {
    ...
    setDivisions(governanceData[0]?.divisions || []);
  }

  const handleCheckboxChange =
    (option: string) => (e: ChangeEvent<HTMLInputElement>) => {
      if (e.target.checked) {
        setState((prevState) => ({ ...prevState, [option]: true }))
        setSelectedOption(option)
        setIsModalOpen(true)
      } else {
        setState((prevState) => ({ ...prevState, [option]: false }))
      }
    };
  const closeModal = () => {
